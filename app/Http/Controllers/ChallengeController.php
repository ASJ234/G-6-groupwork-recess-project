...
class ChallengeController extends Controller
{
    ...
    public function sendChallengeReports($challenge_id)
    {
        ...
        foreach ($challenge->attempts->unique('participant_id') as $attempt) {
            ...
            $attempts = $challenge->attempts->where('participant_id', $participant->id);
            
            // Generate PDF for each participant with their attempts
            $pdf = Pdf::loadView('reports.challenge_results', ['attempts' => $attempts]);

            // Send email with PDF attached
            Mail::to($participant->emailAddress)->send(new \App\Mail\ChallengeResults($pdf));
        }
